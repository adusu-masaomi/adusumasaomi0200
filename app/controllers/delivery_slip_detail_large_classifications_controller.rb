class DeliverySlipDetailLargeClassificationsController < ApplicationController
  ...
  def index
    ...
    if (params[:q].present? && params[:q][:s].present?) || session[:sort_dl] != nil
      ...
      #（モーダル編集、行ソートでこの処理をしないようにしている）
        ...
        if params[:q].present? 
          #if $sort_dl.nil?
          if session[:sort_dl].nil?
            #$sort_dl = "desc"
            session[:sort_dl] = "desc"
          end
  ...
  #納品書データを見積書データへ丸ごとコピー（もっと単純な方法ない？）
    ...
    if @success_flag == true
      ...
      end      #@quotation_header_id.present?
    end        #@success_flag == true
    #
  end
