class QuotationDetailMiddleClassificationsController < ApplicationController
  ...
  def index
    ...
    if (params[:q].present? && params[:q][:s].present?) || session[:sort_qm] != nil
      #order順のパラメータ(asc/desc)がなぜか１パターンしか入らないので、カラム強制にセットする。
      column_name = "line_number"
      
      #if $not_sort_qm != true
      if session[:not_sort_qm] != true
      #ここでにソートを切り替える。（パラメータで入ればベストだが）
      #（モーダル編集、行ソートでこの処理をしないようにしている）
        if params[:q].present? 
          #if $sort_qm.nil?
          if session[:sort_qm].nil?
            #$sort_qm = "desc"
            session[:sort_qm] = "desc"
          end 
 
          #if $sort_qm != "asc"
          if session[:sort_qm] != "asc"
            #$sort_qm = "asc"
            session[:sort_qm] = "asc"
          else
            #$sort_qm = "desc"
  ...
  def edit
    ...
    @working_middle_item = WorkingMiddleItem.all 
    @working_category = WorkingCategory.all
  ...
  def save_price_to_headers_move(line_number)
    ...
    if quotation_header.present? 
      ...
      #upd230616
      total_price_quote = QuotationDetailLargeClassification.where(["quotation_header_id = ?", params[:quotation_header_id]]).sumpriceQuote
      quotation_header.quote_price = total_price_quote
      
      #実行金額
      #quotation_header.execution_amount = execution_total_price_Large
      #upd230616
      total_price_execution = QuotationDetailLargeClassification.where(["quotation_header_id = ?", params[:quotation_header_id]]).sumpriceExecution
      quotation_header.execution_amount = total_price_execution
  ...
  #以下もPrivate下であるが..
    ...
    def save_price_to_large_classifications
      ...
      if @quotation_detail_large_classification.present?
        ...
        #upd230513
        if @quotation_detail_large_classification.quantity.present? &&
           @quotation_detail_large_classification.quantity > 1
        ...
        if @quotation_detail_large_classification.quantity.blank?
          @quotation_detail_large_classification.quantity = 1
          @quotation_detail_large_classification.working_unit_id = 22
          @quotation_detail_large_classification.working_unit_name = "式"
        end
        #
