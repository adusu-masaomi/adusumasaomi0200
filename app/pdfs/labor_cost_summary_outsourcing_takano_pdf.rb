class LaborCostSummaryOutsourcingTakanoPDF
    
  
  #def self.create labor_cost_summary_outsourcing_takano
  def self.create(construction_daily_reports, outsourcing_staff)
  #労務集計表(横)PDF発行
    ...
    # tlfファイルを読み込む
    report = Thinreports::Report.new(layout: "#{Rails.root}/app/pdfs/labor_cost_summary_outsourcing_takano_pdf.tlf")
    ...
    if @flag.nil? 
      ...
      #if $outsourcing_staff == 5
      if outsourcing_staff == 5
        tmp = "小柳￥１７，０００"
        report.page.item(:staff_outsourcing).value(tmp)
      #elsif $outsourcing_staff == 6
      elsif outsourcing_staff == 6
        tmp = "須戸￥１５，０００"
        report.page.item(:staff_outsourcing).value(tmp)
      end
    ...
    #同一日でも社員が変わった場合。
      ...
      when 4 then
        #高野
        @man_month_5 += construction_daily_report.man_month
        @labor_cost_5 += construction_daily_report.labor_cost
      when 5 then
        #小柳さん
      ...
      when 6 then
        #須戸さん
        #注意！須戸・小柳・高野のペアはないとする（仮にあった場合、須戸さんが優先される）
        @man_month_4 += construction_daily_report.man_month
        @labor_cost_4 += construction_daily_report.labor_cost
      end
      ...
      #@construction_daily_report = construction_daily_reports           
  ...
  def self.countTotal
    ...
    #高野
    #トータルへカウント
      @man_month_5_total += @man_month_5
      @labor_cost_5_total += @labor_cost_5
    when 5 
    #小柳さん
    #トータルへカウント
  ...
  #小計の見栄えを調整
    ...
    #高野
    if @man_month_5_total == 0
      @man_month_5_total = ""
      @labor_cost_5_total = ""
    else
      @man_month_5_total = sprintf( "%.3f", @man_month_5_total )
      @labor_cost_5_total = "￥" + @labor_cost_5_total.to_s(:delimited)
    end
  ...
  def self.setRow()
    ...
    if @man_month_5 == 0
      ...
      @tmp_man_month_5 = sprintf( "%.3f", @man_month_5 )
      @tmp_working_date_5 = @tmp_working_date
    end
    if @labor_cost_5 == 0
	    @tmp_labor_cost_5 = ""
    else
      @tmp_labor_cost_5 = "￥" + @labor_cost_5.to_s(:delimited)
    end
