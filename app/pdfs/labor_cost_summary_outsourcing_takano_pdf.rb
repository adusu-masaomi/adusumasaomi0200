	...
	#労務集計表(横)PDF発行
      ...
      $construction_daily_reports.order(:working_date).each do |construction_daily_reports| 
		 ...
		 if @flag.nil? 
           ...
           if $outsoucing_staff == 5
             tmp = "小柳￥１７，０００"
             report.page.item(:staff_outsourcing).value(tmp)
           elsif $outsoucing_staff == 6
             tmp = "須戸￥１５，０００"
             report.page.item(:staff_outsourcing).value(tmp)
           end
           #
		...
		case construction_daily_reports.staff_id
          ...
          when 4 then
		  #高野
		    @man_month_5 += construction_daily_reports.man_month
		    @labor_cost_5 += construction_daily_reports.labor_cost
		  
		  when 5 then
		  #小柳さん
		    @man_month_4 += construction_daily_reports.man_month
		    @labor_cost_4 += construction_daily_reports.labor_cost
