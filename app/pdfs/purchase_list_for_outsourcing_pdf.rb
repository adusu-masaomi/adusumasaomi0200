class PurchaseListForOutsourcingPDF 
  ...
  def self.create(purchase_data, supplier_flag, no_color)
    #仕入表(仕入先別)PDF発行
    ...
    @purchase_amount_tax_in_total = 0  
    ...
    #if !$no_color
    if !no_color
    #ADUSU(社長提示用)用の場合のソート順
      #仕入日・注番でソート
      #sort_key = "purchase_date, purchase_order_code, id"
      #支払予定日、注番でソート
      sort_key = "payment_due_date, purchase_order_code, id"
    else
    ...
    purchase_data.joins(:purchase_order_datum).order(sort_key).each do |purchase_datum|
      ...
      report.list(:default).add_row do |row|
        ...
        get_date_manual(purchase_datum)  
        #
        ...
        end  #if purchase_datum.outsourcing_payment_flag == 1
      end    #end do(report)
    end	     #end do
